...
@testset "Onion.jl" begin
    ...
    @testset "UNet Components" begin
        ...
        @testset "ResidualBlock" begin
            ...
            rb1 = Onion.Unet.ResidualBlock(64)
            x = randn(Float32, 32, 32, 64, 2)
            y1 = rb1(x)
            @test size(y1) == size(x)

            # With time embedding
            rb2 = Onion.Unet.ResidualBlock(64, time_emb=true, emb_dim=256)
        ...
        @testset "EncoderBlock" begin
            eb = Onion.Unet.EncoderBlock(3, 64, time_emb=true, emb_dim=256)
        ...
        @testset "UNet Helpers" begin
            # Test reverse_tuple
            t = (1, 2, 3, 4, 5)
            rt = Onion.Unet.reverse_tuple(t)
            @test rt == (5, 4, 3, 2, 1)

            # Test process_encoders and process_decoders with a mini-model
            enc1 = Onion.Unet.EncoderBlock(3, 16)
            enc2 = Onion.Unet.EncoderBlock(16, 32)
            encoders = (enc1, enc2)

            dec1 = Onion.Unet.DecoderBlock(32, 16)
            dec2 = Onion.Unet.DecoderBlock(16, 8)
            decoders = (dec1, dec2)

            x = randn(Float32, 32, 32, 3, 2)

            # Test process_encoders
            x_out, skips = Onion.Unet.process_encoders(x, encoders)
            @test length(skips) == 2
            @test size(x_out) == (8, 8, 32, 2)

            # Test process_decoders
            rev_skips = Onion.Unet.reverse_tuple(skips)
            y = Onion.Unet.process_decoders(x_out, decoders, rev_skips)
